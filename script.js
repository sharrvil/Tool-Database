document.addEventListener('DOMContentLoaded', function() {
    ...
    function displayTSCodeResults(tsCode, tools) {
        ...
        // Use the same 3-column layout as the tool rows below so "TS Code"
        // lines up with "Tool 1"/"Tool 2" and the code lines up with the
        // tool number column (third cell left blank, no location for this row)
        addToolRow('TS Code', tsCode, '');

        tools.forEach((tool, index) => {
            // tool is { code, location }
            addToolRow(`Tool ${index + 1}`, tool.code, tool.location);
        });
    ...
    function searchByTSCode(tsCode) {
        ...
        fetch(url)
            ...
            .then(csvData => {
                ...
                const toolNumberIndex = headers.findIndex(h =>
                    h.trim().replace(/^"|"$/g, '').toLowerCase() === 'tool number'
                );
                const locationIndex = headers.findIndex(h =>
                    h.trim().replace(/^"|"$/g, '').toLowerCase() === 'location'
                );
                ...
                // Build a lookup map of Tool Number -> Location so each matched
                // tool can show its location alongside it
                const toolLocationMap = {};
                if (toolNumberIndex !== -1 && locationIndex !== -1) {
                    for (let i = 1; i < rows.length; i++) {
                        const rowData = parseCSVRow(rows[i]);
                        const num = rowData[toolNumberIndex]?.replace(/^"|"$/g, '').trim();
                        const loc = rowData[locationIndex]?.replace(/^"|"$/g, '').trim();
                        if (num) {
                            toolLocationMap[num.toUpperCase()] = loc || 'N/A';
                        }
                    }
                }
                ...
                for (let i = 1; i < rows.length; i++) {
                    ...
                    if (sheetCode === searchCode) {
                        ...
                        if (tools) {
                            ...
                            // Pair each tool with its location from the lookup map
                            const toolsWithLocation = toolChunks.map(chunk => {
                                const pureCode = (chunk.match(/^TT\d+/i) || [chunk])[0];
                                const location = toolLocationMap[pureCode.toUpperCase()] || 'N/A';
                                return { code: chunk, location: location };
                            });

                            matchedTools.push(...toolsWithLocation);
                        }
                    }
                }
...
// Search function
    ...
    // Helper to add a tool row with an extra column showing its location,
    // with no header/label on that column - just the value itself.
    // If location is empty (used for the "TS Code" summary row), the third
    // cell is left blank so columns still line up with the tool rows below.
    function addToolRow(label, toolCode, location) {
        const row = document.createElement('div');
        row.className = 'table-row tool-row';
        ...
        codeCell.className = 'table-cell cell-code';
        codeCell.textContent = toolCode;

        const locationCell = document.createElement('div');
        locationCell.className = 'table-cell cell-location';
        if (location) {
            locationCell.innerHTML = `<span class="material-icons location-pin">place</span><span>${location}</span>`;
        }
